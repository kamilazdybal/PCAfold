...
class TestManifoldAssessment(unittest.TestCase):

    def test_average_knn_distance_allowed_calls(self):

        pass

    def test_average_knn_distance_not_allowed_calls(self):

        pass

class TestRegressionAssessment(unittest.TestCase):

    def test_coefficient_of_determination_allowed_calls(self):
    ...
    def test_coefficient_of_determination_not_allowed_calls(self):

        X = np.random.rand(100,5)

        with self.assertRaises(ValueError):
            plt = coefficient_of_determination(X[:,0:2], X[:,1])

        with self.assertRaises(ValueError):
            plt = coefficient_of_determination(X[:,0], X[:,1:3])

        with self.assertRaises(ValueError):
            plt = coefficient_of_determination([1,2,3], X[:,1])

        with self.assertRaises(ValueError):
            plt = coefficient_of_determination(X[:,0], [1,2,3])

    def test_coefficient_of_determination_computation(self):
        ...
        try:
            r2 = coefficient_of_determination(X[:,0], X[:,0])
            self.assertTrue(r2==1)
        except Exception:
            self.assertTrue(False)

        pca_X = PCA(X, scaling='auto', n_components=4)
        X_rec = pca_X.reconstruct(pca_X.transform(X))
        ...
        try:
            r2 = coefficient_of_determination(X[:,0], X[:,1])
            self.assertTrue(r2<1)
        except Exception:
            self.assertTrue(False)
    ...
    def test_mean_squared_error_allowed_calls(self):
        ...
        try:
            MSE = mean_squared_error(X[:,0], X[:,0])
        except Exception:
            self.assertTrue(False)

    def test_mean_squared_error_not_allowed_calls(self):

        X = np.random.rand(100,5)

        with self.assertRaises(ValueError):
            MSE = mean_squared_error(X[:,0:2], X[:,0])

        with self.assertRaises(ValueError):
            MSE = mean_squared_error(X[:,0], X[:,0:2])

        with self.assertRaises(ValueError):
            MSE = mean_squared_error([1,2,3], X[:,0])

        with self.assertRaises(ValueError):
            MSE = mean_squared_error(X[:,0], [1,2,3])

    def test_mean_squared_error_computation(self):
        ...
        try:
            MSE = mean_squared_error(X[:,0], X[:,1])
            self.assertTrue(MSE>0)
        except Exception:
            self.assertTrue(False)

    def test_root_mean_squared_error_allowed_calls(self):
        ...
        try:
            RMSE = root_mean_squared_error(X[:,0], X[:,0])
        except Exception:
            self.assertTrue(False)

    def test_root_mean_squared_error_not_allowed_calls(self):

        X = np.random.rand(100,5)

        with self.assertRaises(ValueError):
            RMSE = root_mean_squared_error(X[:,0:2], X[:,0])

        with self.assertRaises(ValueError):
            RMSE = root_mean_squared_error(X[:,0], X[:,0:2])

        with self.assertRaises(ValueError):
            RMSE = root_mean_squared_error([1,2,3], X[:,0])

        with self.assertRaises(ValueError):
            RMSE = root_mean_squared_error(X[:,0], [1,2,3])

    def test_root_mean_squared_error_computation(self):
        ...
        try:
            RMSE = root_mean_squared_error(X[:,0], X[:,1])
            self.assertTrue(RMSE>0)
        except Exception:
            self.assertTrue(False)

    def test_normalized_root_mean_squared_error_allowed_calls(self):
        ...
        try:
            NRMSE = normalized_root_mean_squared_error(X[:,0], X_rec[:,0])
        except Exception:
            self.assertTrue(False)

        try:
            NRMSE = normalized_root_mean_squared_error(X[:,0], X[:,0])
        except Exception:
            self.assertTrue(False)

    def test_normalized_root_mean_squared_error_not_allowed_calls(self):

        X = np.random.rand(100,5)

        with self.assertRaises(ValueError):
            NRMSE = normalized_root_mean_squared_error(X[:,0:2], X[:,0])

        with self.assertRaises(ValueError):
            NRMSE = normalized_root_mean_squared_error(X[:,0], X[:,0:2])

        with self.assertRaises(ValueError):
            NRMSE = normalized_root_mean_squared_error([1,2,3], X[:,0])

        with self.assertRaises(ValueError):
            NRMSE = normalized_root_mean_squared_error(X[:,0], [1,2,3])

    def test_normalized_root_mean_squared_error_computation(self):
        ...
        try:
            NRMSE = normalized_root_mean_squared_error(X[:,0], X[:,1])
            self.assertTrue(NRMSE>0)
        except Exception:
            self.assertTrue(False)

    def test_turning_points_allowed_calls(self):

        pass

    def test_turning_points_not_allowed_calls(self):

        pass

    def test_good_estimate_allowed_calls(self):

        pass

    def test_good_estimate_not_allowed_calls(self):

        pass

    def test_good_direction_estimate_allowed_calls(self):
        ...
        try:
            (GD, GDE) = good_direction_estimate(X, X_rec)
        except Exception:
            self.assertTrue(False)

        try:
            (GD, GDE) = good_direction_estimate(X[:,0:2], X_rec[:,0:2])
        except Exception:
            self.assertTrue(False)

    def test_good_direction_estimate_not_allowed_calls(self):
        ...
        with self.assertRaises(ValueError):
            (GD, GDE) = good_direction_estimate(X[:,0], X_rec[:,0:2])

        with self.assertRaises(ValueError):
            (GD, GDE) = good_direction_estimate(X[:,0:2], X_rec[:,0])

        with self.assertRaises(ValueError):
            (GD, GDE) = good_direction_estimate(X[:,0:2], X_rec[:,0:1])

        with self.assertRaises(ValueError):
            (GD, GDE) = good_direction_estimate(X[:,0:1], X_rec[:,0:1])

        with self.assertRaises(ValueError):
            (GD, GDE) = good_direction_estimate(X[:,0:2], [1,2,3])

        with self.assertRaises(ValueError):
            (GD, GDE) = good_direction_estimate([1,2,3], X_rec[:,0:2])

        with self.assertRaises(ValueError):
            (GD, GDE) = good_direction_estimate(X, X_rec, tolerance=[1])

    def test_good_direction_estimate_computation(self):
        ...
        try:
            (GD, GDE) = good_direction_estimate(X, X_rec, tolerance=tolerance)
        ...
        try:
            (GD, GDE) = good_direction_estimate(X, X_rec, tolerance=0.001)
            self.assertTrue(GDE<100)
            self.assertTrue(np.all(GD<=1))
        except Exception:
            self.assertTrue(False)

    def test_plot_2d_regression_allowed_calls(self):
        ...
        try:
            plt = plot_2d_regression(X[:,1], X[:,0], X_rec[:,0], x_label='$x$', y_label='$y$', figure_size=(7,7), title='2D regression')
            plt.close()
        except Exception:
            self.assertTrue(False)

        try:
            plt = plot_2d_regression(X[:,1:2], X[:,0:1], X_rec[:,0:1], x_label='$x$', y_label='$y$', figure_size=(7,7), title='2D regression')
            plt.close()
        except Exception:
            self.assertTrue(False)

    def test_plot_2d_regression_not_allowed_calls(self):

        X = np.random.rand(100,5)

        with self.assertRaises(ValueError):
            plt = plot_2d_regression(X[:,0:2], X[:,1], X[:,2])
            plt.close()

        with self.assertRaises(ValueError):
            plt = plot_2d_regression(X[:,0], X[:,1:3], X[:,2])
            plt.close()

        with self.assertRaises(ValueError):
            plt = plot_2d_regression(X[:,0], X[:,1], X[:,2:4])
            plt.close()

        with self.assertRaises(ValueError):
            plt = plot_2d_regression([1,2,3], X[:,1], X[:,1])
            plt.close()

        with self.assertRaises(ValueError):
            plt = plot_2d_regression(X[:,0], [1,2,3], X[:,1])
            plt.close()

        with self.assertRaises(ValueError):
            plt = plot_2d_regression(X[:,0], X[:,1], [1,2,3])
            plt.close()

        with self.assertRaises(ValueError):
            plt = plot_2d_regression(X[:,0], X[:,1], X[:,1], x_label=[1])
            plt.close()

        with self.assertRaises(ValueError):
            plt = plot_2d_regression(X[:,0], X[:,1], X[:,1], y_label=[1])
            plt.close()

        with self.assertRaises(ValueError):
            plt = plot_2d_regression(X[:,0], X[:,1], X[:,1], figure_size=[1])
            plt.close()

        with self.assertRaises(ValueError):
            plt = plot_2d_regression(X[:,0], X[:,1], X[:,1], title=[1])
            plt.close()

    def test_plot_3d_regression_allowed_calls(self):
    ...
    def test_plot_3d_regression_not_allowed_calls(self):
        ...
        with self.assertRaises(ValueError):
            plt = plot_3d_regression([1,2,3], X[:,1], X[:,2], X[:,2])
            plt.close()

        with self.assertRaises(ValueError):
            plt = plot_3d_regression(X[:,0], [1,2,3], X[:,2], X[:,2])
            plt.close()

        with self.assertRaises(ValueError):
            plt = plot_3d_regression(X[:,0], X[:,1], [1,2,3], X[:,2])
            plt.close()

        with self.assertRaises(ValueError):
            plt = plot_3d_regression(X[:,0], X[:,1], X[:,2], [1,2,3])
            plt.close()

        with self.assertRaises(ValueError):
            plt = plot_3d_regression(X[:,0], X[:,1], X[:,2], X[:,2], x_label=[1])
            plt.close()

        with self.assertRaises(ValueError):
            plt = plot_3d_regression(X[:,0], X[:,1], X[:,2], X[:,2], y_label=[1])
            plt.close()

        with self.assertRaises(ValueError):
            plt = plot_3d_regression(X[:,0], X[:,1], X[:,2], X[:,2], z_label=[1])
            plt.close()

        with self.assertRaises(ValueError):
            plt = plot_3d_regression(X[:,0], X[:,1], X[:,2], X[:,2], figure_size=[1])
            plt.close()

        with self.assertRaises(ValueError):
            plt = plot_3d_regression(X[:,0], X[:,1], X[:,2], X[:,2], title=[1])
            plt.close()
