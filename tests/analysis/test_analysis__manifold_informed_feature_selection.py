...
# ------------------------------------------------------------------------------
    ...
    def test_analysis__manifold_informed_feature_selection__not_allowed_calls(self):
        ...
        # Wrong type:
        with self.assertRaises(ValueError):
            (selected_variables, costs) = analysis.manifold_informed_feature_selection(X, X_source, variable_names, scaling, bandwidth_values, d_hat_variables=[1], direct_integration=True)

        with self.assertRaises(ValueError):
            (selected_variables, costs) = analysis.manifold_informed_feature_selection(X, X_source, variable_names, scaling, bandwidth_values, add_transformed_source=[1], direct_integration=True)

        with self.assertRaises(ValueError):
            (selected_variables, costs) = analysis.manifold_informed_feature_selection(X, X_source, variable_names, scaling, bandwidth_values, target_manifold_dimensionality=[1], direct_integration=True)

        with self.assertRaises(ValueError):
            (selected_variables, costs) = analysis.manifold_informed_feature_selection(X, X_source, variable_names, scaling, bandwidth_values, bootstrap_variables=1, direct_integration=True)

        with self.assertRaises(ValueError):
            (selected_variables, costs) = analysis.manifold_informed_feature_selection(X, X_source, variable_names, scaling, bandwidth_values, weight_area=[1], direct_integration=True)
        ...
        with self.assertRaises(ValueError):
            (selected_variables, costs) = analysis.manifold_informed_feature_selection(X, X_source, variable_names, scaling, bandwidth_values, direct_integration=True, verbose=[1])

        with self.assertRaises(ValueError):
            (selected_variables, costs) = analysis.manifold_informed_feature_selection([1], X_source, variable_names, scaling, bandwidth_values, direct_integration=True)

        with self.assertRaises(ValueError):
            (selected_variables, costs) = analysis.manifold_informed_feature_selection(X, [1], variable_names, scaling, bandwidth_values, direct_integration=True)

        with self.assertRaises(ValueError):
            (selected_variables, costs) = analysis.manifold_informed_feature_selection(X, X_source, 'A', scaling, bandwidth_values, direct_integration=True)

        with self.assertRaises(ValueError):
            (selected_variables, costs) = analysis.manifold_informed_feature_selection(X, X_source, variable_names, [1], bandwidth_values, direct_integration=True)

        with self.assertRaises(ValueError):
            (selected_variables, costs) = analysis.manifold_informed_feature_selection(X, X_source, variable_names, scaling, [1], direct_integration=True)
