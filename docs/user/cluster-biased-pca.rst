...
``cluster_biased_pca.py`` module contains functions for performing analysis of
data sets using cluster-biased PCA. Cluster-biased PCA is a link between local
PCA and global PCA - it is using local features to inform (bias) the global PCs
through changing sample densities.
